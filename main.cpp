...
int main(){
	...
	while(!WindowShouldClose()){
		...
		if(GameState==0){
			...
			for(int i=0;i<sitem.size();i++)sitem.erase(sitem.begin()+i);
			...
			if(!Pause){
				...
				for(int i=0;i<jiege.size();i++){
					...
					if(jiege[i].blood<=0){
						sitem.push_back(SItem(GetRandomValue(jiege[i].x-1,jiege[i].x+1),1,GetRandomValue(1,2)));/*GetRandomValue(1,3)*/
				...
				for(int i=0;i<cxk.size();i++){
					...
					if(cxk[i].blood<=0){
						sitem.push_back(SItem(GetRandomValue(cxk[i].x-1,cxk[i].x+1),2,GetRandomValue(1,2)));//调整掉落物的生成数量
				...
				if(GetRandomValue(0,30)==15){//每一帧1/50的刷怪概率
				...
				if(player1.blood<=0)GameState=2;
				//---------------key ctrl-------------------------- 
			}else{
				for(int i=0;i<=floor(GetScreenWidth()/block.height);i++){//横行
					DrawTexture(block,i*block.height,GROUND+BW*(submap[(floor(GetScreenWidth()/block.height)/2)]-submap[i]),WHITE);//公式:GR+bw(submap[*(the base of the map)]-submap[i])=y
